...
| Status flags | None |
| Exceptions | None in v0.1 |
| Reset state | `pc = 0`; register and data memory reset values are implementation-defined unless a profile says otherwise |
...
Reserved fields must be encoded as zero. Reserved classes and operations are
invalid encodings. A v0.1 implementation may treat invalid encodings as no-ops,
but portable software must not rely on that behavior.
...
Shift and rotate amounts are reduced modulo 16 because the data path is 16 bits
wide.
